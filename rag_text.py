...
Problem definition
------------------
  Objective  : MINIMIZE total GWP  (lb CO2-eq / yd3)
...
------------------
  ...
  and each iteration logs:
    gwp_gap   = current_gwp - ga_ref_gwp   (how far from optimal, lb)
...
STAG_THRESHOLD  = 1.0       # lb — improvement < this = no progress
...
# Anti-oscillation
ANTI_OSC_WINDOW = 3
ANTI_OSC_TOL    = 5.0       # lb per ingredient
...
def run_ga(raw_b: dict, der_b: dict, meta: dict,
    ...
    if best:
        print(f"[GA] Best: GWP={best['gwp']:.2f} lb/yd3  "
...
def retrieve_similar_mixes(current_mix: dict, df: pd.DataFrame,
                            k: int = 3, pool: str = "feasible") -> list:
    """
    Dynamic RAG: retrieve k most similar mixes from dataset
    based on normalized Euclidean distance in ingredient space.
    pool="feasible": only rows satisfying STRENGTH_MIN
    pool="full":     all 726 rows
    """
...
====================
OBJECTIVE  : MINIMISE total embodied carbon (GWP, lb CO2-eq/yd3) — as low as possible
CONSTRAINT : Predicted 28-day compressive strength >= {strength_min} MPa (HARD LIMIT)
...
Each material affects both GWP and 28-day strength. Use this table every step:
  ...
  WATER
       ...
       Strategy   : Reduce water to improve strength without raising GWP.
                    Reducing WATER by 10 lb (lower w/b) can recover strength
  ...
  WR / WR_HR  (Water reducer / Superplasticiser)
       ...
       Strategy   : High WR+WR_HR (50-150 lb total) is essential for Path 2.
                    If you want to reduce total binder below 420 lb,
                    you MUST increase WR/WR_HR to compensate.
                    WR range: 0-211 lb. WR_HR range: 0-127 lb.

  AEA  (Air-entraining admixture)
       GWP factor : 0.000  ZERO
       Strength   : Slight negative at high doses (air voids reduce strength)
       Strategy   : Keep AEA low (0-5 lb). Not a useful optimisation lever.

  ACC  (Accelerator)
       GWP factor : 0.000  — zero GWP impact
       Strength   : Direct positive effect on both 7d and 28d strength.
                    Especially useful when total binder is low.
       Strategy   : ACC 200-750 lb can compensate for strength loss when
                    reducing total binder on Path 2. Zero GWP cost.
                    If strength is below floor and you do not want to add PC,
                    try ACC 100-400 lb first.

MOST EFFICIENT MOVES (ranked by CO2 saving per unit):
  1. Replace 10 lb PC with 10 lb SC  → saves 7.84 lb CO2, mild strength loss
  2. Reduce WATER by 10 lb           → saves 0 lb CO2, RECOVERS strength
  3. Replace 10 lb PC with 10 lb FA  → saves 7.20 lb CO2, more strength loss than SC

VARIABLE BOUNDS  (lb/yd3, from dataset)
=======================================
{raw_bounds}
...
SITUATION D: Stuck in local optimum (GWP not improving for several iters)
  ...
  -> Options (try in this order):
       ...
       Option 1 — Switch to Path 2 (most impactful, often missed):
         Set total binder (PC+SC+FA) < 420 lb,
         increase FAGG, CAGG
...
GENERAL PRINCIPLES:
  ...
  - WR/WR_HR should not exceed 200 lb total
  - Make changes of at least 15 lb per iteration — small tweaks stall progress
...
GWP breakdown:
{gwp_breakdown}
  TOTAL GWP : {gwp:.2f} lb CO2-eq/yd3
...
=== PROGRESS vs PREVIOUS ITERATION ===
  Previous iter {prev_iter}  : GWP={prev_gwp:.2f} lb   28d={prev_28:.2f} MPa
  This iter     {it}  : GWP={gwp:.2f} lb   28d={p28:.2f} MPa
  GWP change          : {gwp_change:+.2f} lb   {gwp_trend}
...
Best solution so far (iter {best_iter}):
  ...
  -> GWP={best_gwp:.2f} lb   28d={best_28:.2f} MPa
...
Your current vs best (all variables):
  ...
  GWP: {gwp:.2f} vs {best_gwp:.2f} ({gwp_vs_best:+.2f} lb)
...
def _build_system_prompt(raw_b, der_b, few_shot, strength_min, memory=None):
    ...
    for ex in few_shot:
        ...
        fs_parts.append(
            f"[{ex['label']}]\n  {mix_str}\n"
            f"  -> 28d={ex['pred_28day']} MPa   GWP={ex['gwp']} lb CO2/yd3"
        )
...
def _build_feedback(it, max_it, mix, preds, gwp, feas,
    ...
    gwp_breakdown = "\n".join(
        f"  {k:<6} {mix.get(k,0):6.1f} lb x {GWP_FACTORS[k]:.4f} = {c:6.2f} lb CO2"
    ...
    if gwp_change < -0.5:
        gwp_trend = f"DECREASED {abs(gwp_change):.2f} lb -- good progress"
    elif gwp_change > 0.5:
        gwp_trend = f"INCREASED {gwp_change:.2f} lb -- WRONG DIRECTION"
    else:
    ...
    if len(recent) == ANTI_OSC_WINDOW:
        ...
        if oscillating:
            if scm_near_limit:
                ...
                osc_warning = (
                    "*** OSCILLATION WARNING ***\n"
                    "Last 3 proposals nearly identical. Change at least 2 variables by > 20 lb.\n\n"
                )
    ...
    if not feas["feasible"]:
        ...
        if gwp_change > 0.5:
            fb.append(
                f"  GWP went UP by {gwp_change:.2f} lb — this is the wrong direction.\n"
                "  You must reduce GWP. Use the decision table:\n"
                "    - Replace more PC with SC (saves 0.784 lb CO2 per lb swapped)\n"
                "    - Reduce WATER to recover any strength lost (zero GWP cost)"
            )
        elif abs(gwp_change) <= 0.5:
            fb.append(
                f"  GWP barely changed ({gwp_change:+.2f} lb). You need a BIGGER move.\n"
                "  Try replacing PC with SC in increments of 20-30 lb at a time."
            )
        else:
            fb.append(f"  GWP decreased {abs(gwp_change):.2f} lb — good. Keep going.")
        ...
        if str_margin > 8 and gwp > best_gwp + 5:
            fb.append(
                ...
                f"  Your current GWP ({gwp:.1f}) is {gwp - best_gwp:.1f} lb above best.\n"
                f"  Target: bring strength DOWN towards {STRENGTH_MIN} MPa.\n"
                f"  Options:\n"
                f"    - Reduce PC by 20 lb AND SC by 20 lb (saves ~{20 * 1.048 + 20 * 0.264:.1f} lb CO2)\n"
                f"    - Increase FAGG+CAGG by 200 lb (dilutes binder, reduces total paste)\n"
                f"    - Reduce total binder below 420 lb, compensate with ACC or WR increase"
            )
        elif str_margin > 3:
            fb.append(
                f"  Strength margin is {str_margin:.1f} MPa — reasonable headroom.\n"
                f"  Options:\n"
                f"    - Swap 15 lb PC -> SC (saves ~{15 * 0.784:.1f} lb CO2)\n"
                f"    - Increase WR by 30 lb to allow cutting WATER by 15 lb\n"
                f"    - Try adding ACC 100-200 lb to create room for more PC reduction"
            )
        elif str_margin < 2:
            fb.append(
                f"  Strength margin is only {str_margin:.1f} MPa — very tight.\n"
                "  Do NOT reduce PC further. Instead:\n"
                "    - Reduce WATER by 5-10 lb to recover strength margin\n"
                "    - Then try another PC->SC swap"
            )
        ...
        if cur_wr > 150:
            fb.append(
                f"  WR+WR_HR = {cur_wr:.0f} lb — already very high. Do NOT increase further."
            )
        elif cur_wr < 50 and mix.get("WATER", 200) > 170:
            fb.append(
                f"  WR+WR_HR = {cur_wr:.0f} lb — currently low.\n"
                f"  Consider increasing WR to 50-100 lb: this allows reducing WATER\n"
                f"  by 20-30 lb, recovering strength at zero GWP cost."
            )
        ...
        if cur_acc < 100 and str_margin < 3:
            fb.append(
                f"  ACC = {cur_acc:.0f} lb — very low.\n"
                f"  Adding ACC 200-400 lb boosts strength at zero GWP cost.\n"
        ...
        if cur_fagg + cur_cagg < 2000 and str_margin > 5:
            fb.append(
                f"  FAGG+CAGG = {cur_fagg + cur_cagg:.0f} lb — consider increasing.\n"
    ...
    rag_block = ""
    if df is not None:
        similar = retrieve_similar_mixes(mix, df, k=5, pool="feasible")
        if similar:
            rag_lines = [
            ...
            for i, s in enumerate(similar, 1):
                ...
                rag_lines.append(
                    f"  [{i}] A mix with {s.get('PC', 0):.0f} lb/yd3 Portland cement, "
                    f"{s.get('SC', 0):.0f} lb/yd3 slag cement, and {s.get('FA', 0):.0f} lb/yd3 "
                    f"fly ash achieves {s['pred_28day']:.1f} MPa 28-day strength "
                    f"with GWP of {s['gwp']:.1f} lb CO₂/yd3. "
                    f"Total binder: {tb:.0f} lb/yd3, w/b ratio: {wb:.2f}, "
                    f"FAGG: {s.get('FAGG', 0):.0f}, CAGG: {s.get('CAGG', 0):.0f}, "
                    f"ACC: {s.get('ACC', 0):.0f}, WR: {s.get('WR', 0):.0f} lb/yd3.\n"
                )
            rag_block = "\n".join(rag_lines)
    fb.append(rag_block)
...
def format_memory_for_prompt(memory: list) -> str:
    ...
    for rec in memory:
        ...
        lines.append(
            f"  Best achieved: GWP={rec['best_gwp']:.2f} lb  "
        ...
        if rec.get("ga_best"):
            ...
            if rec.get("gwp_gap_to_ga") is not None:
                ...
                if gap > 0:
                    lines.append(
                        f"  LLM was {gap:.2f} lb ABOVE GA last run — "
                        f"try to close or beat this gap."
                    )
                else:
                    lines.append(
                        f"  LLM BEAT GA by {abs(gap):.2f} lb last run — "
...
Try ONE of these escape strategies:
  ...
  C -- Reduce total binder by 80-120 lb, compensate with lower w/b
...
def run_llm(raw_b, der_b, meta, ga_ref, few_shot, memory,
    ...
    print(f"  Objective  : minimize GWP  (lb CO2-eq/yd3)")
    print(f"  Constraint : 28-day >= {STRENGTH_MIN} MPa")
    if ga_ref:
        ...
        if not feas["feasible"]:
            ...
            if feas["str_v"]:
                issues.append(
                    ...
                    f"  Fix: increase PC by 20-30 lb, OR reduce WATER by 10-15 lb, "
                    f"OR add ACC 200-400 lb."
                )
            for v, info in feas["der_v"].items():
                issues.append(
    ...
    Example: "Reducing PC by 20 lb and increasing SC by 20 lb saved ~15 lb GWP 
    ...
    if feasible_all:
        ...
        print(f"\n  {'=' * 50}")
        print(f"  BEST SOLUTION (GWP={best['gwp']:.2f} lb, "
...
def build_report(trajectory: list, ga_ref: dict) -> str:
    ...
    if best:
        ...
        lines += [
            sep,
            f"    GWP gap to GA    : {best['gwp'] - ga_gwp:+.2f} lb CO2/yd3",
...
def main():
    global STRENGTH_MIN, MAX_ITERATIONS, MAX_RESTARTS
    ...
    run_folder = f"run_s{int(STRENGTH_MIN)}_i{MAX_ITERATIONS}_ragtext_{run_id}"
